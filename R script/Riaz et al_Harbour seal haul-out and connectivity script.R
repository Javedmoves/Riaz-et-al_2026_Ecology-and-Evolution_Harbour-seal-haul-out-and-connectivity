...
geo_mean_ci <- function(x, conf.level = 0.95) {
  x <- x[!is.na(x) & x > 0]  # Geometric mean only works for positive numbers
  log_x <- log(x)            # Transform to log-scale
  mean_log <- mean(log_x)
...
# unique IDs shared by both data sets 
ids <- intersect(sf_raw$id, sf_frame_cleaned$id)

gps_col    <- "#6A0DAD"   # purple
haulout_col<- "#FF7F00"   # orange
...
sf_frame_cleaned <- sf_frame_cleaned %>%
  mutate(
    ...
    popup = tooltip  # Same content for popup
  )
...
#   stroke      = TRUE, fillOpacity = 0.8,
#   label       = ~lapply(tooltip, htmltools::HTML),  # ← updated for hover
...
# Update the Type column where id & Num match the 'changes' list
...
# Loop through each ID and save a separate leaflet HTML map
for (this_id in ids) {
  
  # Filter for current ID
  ...
  m <- leaflet(data = sf_id) %>%
    ...
    addCircleMarkers(
      ...
      color       = ~pal_type(Type),
      stroke      = TRUE, fillOpacity = 0.8,
      label       = ~lapply(tooltip, htmltools::HTML),  # Hover label
      popup       = ~htmltools::HTML(tooltip)           # Click popup
    ) %>%
    addLegend(
      position = "bottomright",
      pal = pal_type,
...
InsetMap <- basemap(limits = c(5.5, 23, 53, 72), bathymetry = FALSE , rotate = TRUE, grid.col = NA, land.col = "grey90") +
  ...
  theme(
    panel.border = element_rect(fill = NA, colour = "black", size = 1.3),  # Thicker black border
...
# Order of fylkesnavn (based on your data)
fylkes_levels <- c(
...
# Your color palette (18 colors total)
fylkes_colors_raw <- c(
  "#F39B7FFF", "#7E6148FF", "#00A087FF", "#3C5488FF", "orange",     # Telemark (forced to orange)
  "#C43C4EFF", "black", "#8491B4FF", "#91D1C2FF", "red4", 
  "#4DBBD5FF", "#3B3B3BFF", "#EFC000FF", "#E64B35FF", "#7AA6DCFF",
  "#8F7700FF", "#5C126EFF", "#0073C2FF"  # Aust-Agder forced to purple, black for Akershus
)
...
# Adjust Y positions for specific abbreviations
NorwayLandMap_crop <- NorwayLandMap_crop %>%
  mutate(
    Y = case_when(
...
# arrow_dx <- 0.3  # horizontal shift
# arrow_dy <- 0.3  # vertical shift
# 
...
## Save the figure and add configure arrows manually in Inkscape
...
tiff("FigS1.tiff", width = 6, height = 7, units = 'in', res = 300)
FigS1_MS
dev.off()
...
# Extract the first recorded management area and location for each seal (tagging location)
...
# Function to calculate distance between tagging site and haul-out location
...
# Reorder the 'Month' factor so that it starts from August (8) and ends at January (1)
...
# Create the NewID column based on unique ID within each deployment_management_region
HauloutModelFrame <- HauloutModelFrame %>%
  group_by(deployment_management_region) %>%
...
# Summarize the proportions of Cross_Boundary_Management for each deployment_management_region
...
# Step 2: Make sure NewID order matches y-scale
...
# Filter month_labels to keep only the months we want to show
...
# Define the legend order manually
...
tiff("Fig2.tiff", width = 9, height = 6, units = 'in', res = 300)
Fig2B
dev.off()
...
tiff("Fig2_SUP.tiff", width = 9, height = 6, units = 'in', res = 300)
Jan_HuntingPlot
dev.off()
...
# FirstDummy <- CoxModelFrame %>%
#   filter(NewID == "Vestfold_5"|
#            NewID == "Vestfold_6" |
#            NewID == "Vestfold_7") %>%
#   group_by(NewID) %>%
#   filter(S_DATE  <= min(S_DATE)) %>%
#    mutate(S_DATE = S_DATE - hours(24)) %>%
#    mutate(E_DATE = E_DATE - hours(24)) %>%
#   mutate(Type = "Tag_Deployment",
#          Cross_Boundary_Management = 0,
#          fylkesnavn = deployment_management_region)
...
CoxModelFrame <- CoxModelFrame %>%
  arrange(NewID, S_DATE) %>%  # Ensure data is ordered by seal and time
  group_by(NewID) %>%  # Process each seal separately
  mutate(
...
# SealSurvival <- SealSurvival %>% 
#   mutate(round = round(SurvivalTime, 1))
...
Fig3B <- ggplot(coeff_cox_survival, aes(x=reorder(prev_region, desc(prev_region)), y=DCV.Coef, fill=prev_region))+
  ...
  scale_fill_manual(values= c("#1E90FF", "orange", "#5C126EFF", "#009E73", "yellow2" , "#C43C4EFF", "#000004FF" ))+
...
tiff("Fig3.tiff", width = 8, height = 6, units = 'in', res = 300)
Fig3
dev.off()
...
Spattest <- HauloutModelFrame %>% 
  ...
  st_as_sf(coords = c("X", "Y"), crs = 4326) %>%   # Define original CRS
  st_transform(crs = 32632) %>% 
  mutate(
    X = st_coordinates(.)[,1],  # X coordinate (meters)
    Y = st_coordinates(.)[,2]   # Y coordinate (meters)
  )
...
##############################################################################################################################################
## Calculate some spatial network measures
##############################################################################################################################################

## Number of individuals within each node
node_sizes <- DT %>%
  group_by(group) %>%
  summarise(num_individuals = n_distinct(ID))  # Count unique individuals per cluster

## Weighted edges per ID
movements <- DT %>%
  arrange(ID) %>%  # Ensure data is in chronological order
  group_by(ID) %>%
  mutate(next_group = lead(group)) %>%  # Find the next visited cluster
  filter(!is.na(next_group) & group != next_group) %>%  # Keep only actual movements
  count(group, next_group, name = "weight")  # Count transitions

individual_transitions <- DT %>%
  arrange(ID, datetime1) %>%  # Ensure data is in time order
  group_by(ID) %>%
...
edges <- movements %>% ## There are 21 edges
  rowwise() %>%
  mutate(edge = paste(sort(c(group, next_group)), collapse = "-")) %>% 
  group_by(edge) %>%
...
nodes <- DT_Centroid %>%
  ...
  replace_na(list(num_individuals = 1))  # Assign a small size for unrecorded clusters
...
nodes <- nodes %>%
  left_join(node_sizes, by = "group")  # Adds num_individuals to nodes
...
nodes_sf <- st_as_sf(nodes, coords = c("centroid_lon", "centroid_lat"), crs = 4326)

# Convert CRS for plot
...
distinct_palette <- c(
  ...
  "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A", "#FFFF99", "#B15928", "grey99"
)

distinct_palette <- c("#A6CEE3", "#1F78B4", "#B2DF8A", "#33A02C", "#FB9A99", "#E31A1C", "#FDBF6F", "#FF7F00", "#CAB2D6", "#6A3D9A" ,"#FFFF99" ,"#B15928", "grey99")

distinct_pub_palette <- c(
  "#4477AA",  # blue
  "#117733",  # dark green
  "#E31A1C",  # khaki
  "pink",  # reddish pink
  ...
  "#E69F00",  # orange
  "#F0E442",  # yellow
  "#FFFFFF"   # white (or consider a pale grey like "#DDDDDD" for better visibility)
)

Fig_4 <- BASEMAP1 +
  
  # Edges (curved grey lines, weighted using linewidth)
  geom_curve(
    data = edges_plot,
    aes(x = from_lon, y = from_lat, xend = to_lon, yend = to_lat, linewidth = weight),
    color = "grey40",
    curvature = 0.2,
    alpha = 0.6
  ) +
  scale_linewidth_continuous(
    name = "Edge density",
    range = c(0.3, 2.6),
    breaks = c(1, 2, 4, 5, 6, 7, 11, 34),
    guide = guide_legend(override.aes = list(
      color = "grey40",
      linewidth = c(0.2, 0.5, 0.9, 1.2, 1.5, 1.8, 3, 4.0)  # Manually set widths for better contrast
    ))
  ) +
  
  ggnewscale::new_scale("size") +  # Enable second size scale
  
  # Nodes: size by Degree, fill by group color
  geom_spatial_point(
    data = nodes,
    aes(x = centroid_lon, y = centroid_lat, size = Degree, fill = group),
    shape = 21,
    colour = "black",
    stroke = 1,
    alpha = 0.9
  ...
  # Custom fill colors without legend
  scale_fill_manual(
    values = distinct_pub_palette, 
    guide = "none"
  ) +
